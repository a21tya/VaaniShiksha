...
export default function AlphabetPractice({ language, selected }: { language: string; selected: string }) {
  ...
  function listen() {
    ...
    const voice = window.speechSynthesis.getVoices().find(voice => voice.localService && voice.lang.toLowerCase().startsWith(language === "Hindi" ? "hi" : "en"));
    if (!voice) { setMessage("No installed offline voice for this language. Install a device speech voice in system settings, then reopen this page."); return; }
