...
import { getSavedLessonById, getSavedLessons } from "@/lib/storage";
import { DEMO_LEARNING_KIT, DEMO_SAVED_LESSON, DEMO_SAVED_LESSON_COUNTING, DEMO_SAVED_LESSON_ANIMALS } from "@/lib/demo-data";
...
/** Synchronously resolve demo lesson data by id (no async/IndexedDB needed) */
function getDemoLesson(id: string | null): SavedLesson | null {
  if (!id) return null;
...
export default function StudentModeContent({ lessonId: serverLessonId }: { lessonId: string | null }) {
  const searchParams = useSearchParams();
  const id = searchParams.get("id") ?? serverLessonId;
  const demoLesson = getDemoLesson(id);

  const [dbSavedLesson, setDbSavedLesson] = useState<SavedLesson | null>(null);
  const [dbKit, setDbKit] = useState<LearningKit | null>(null);
  const [currentStep, setCurrentStep] = useState(1);
  const [nextLessonId, setNextLessonId] = useState<string | null>(null);

  // Derive the active data: prefer the demo lesson if matched, else use the loaded DB lesson, else fallback.
  const savedLesson = demoLesson ?? dbSavedLesson;
  const kit = demoLesson?.kit ?? dbKit ?? DEMO_LEARNING_KIT;
  ...
  useEffect(() => {
    ...
    const fetchLesson = async () => {
      try {
        const allLessons = await getSavedLessons();
        if (id) {
          const loaded = await getSavedLessonById(id);
          if (loaded && mounted) {
            setDbSavedLesson(loaded);
            setDbKit(loaded.kit);
        ...
        } else if (mounted && allLessons.length > 0) {
          setNextLessonId(allLessons[0].id);
        }
      } catch (e) {
        console.error(e);
      }
            ...
            <>
              <div className="bg-white rounded-3xl border-2 border-emerald-300 p-8 text-center space-y-4 shadow-sm">
                ...
                <div className="pt-3 flex flex-col sm:flex-row items-center justify-center gap-3">
                  ...
                  <Link href="/student/catalog" className="w-full sm:w-auto px-6 py-3 text-xs sm:text-sm font-bold text-slate-700 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl transition-colors shadow-2xs">📚 Return to Catalog</Link>
                  {nextLessonId && (
                    <Link href={`/student?id=${nextLessonId}`} className="w-full sm:w-auto px-6 py-3 text-xs sm:text-sm font-bold text-white bg-emerald-700 hover:bg-emerald-800 rounded-xl transition-colors shadow-2xs">Next Lesson ➔</Link>
                  )}
                </div>
              </div>
            </>
